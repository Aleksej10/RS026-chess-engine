...
void MainWindow::setHighlight(int x, int y) {
    ...
    auto par = MainWindow::translate(x,y);
    tw->item(par.first, par.second)->setBackgroundColor(QColor(51, 51, 0));
...
void MainWindow::playSelection(int x, int y){
    for (auto m: moves){
        if ((m.x1 == x) && (m.y1 == y)){
            ...
            QMediaPlayer *player = new QMediaPlayer;
            // ...
            player->setMedia(QUrl::fromLocalFile(QDir::currentPath() + "/arrow.mp3"));
            player->setVolume(50);
            player->play();
...
void MainWindow::playSelection(int x, int y){
    ...
    if(k==1){
            move1 m = move1::stupidMode(initial,1).first;
            move1::playMove(initial,m);
