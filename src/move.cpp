...
void prom(std::vector<move1> &moves, const board &pos, int x, int y, int x1, int y1, char side){
    //adds promotion moves to the vector
    int sde;
    int dif = 'a' - 'A';
    sde = (side=='K') ? 0 : 1;
    dif = sde*dif;
...
void move1::playMove(board &pos, const move1 &mov) {
    int side;
    int dif = 'a' - 'A';
    if(pos.flags[0]=='K'){ //switches sides each turn
        side = 0;
        pos.flags[0]='k';
    }else{
        side = 1;
    ...
    if(mov.type=='e'){ //two square pawn move
        pos.flags[5]='a' + mov.y;
        pos.pos[mov.x][mov.y]='_';
        pos.pos[mov.x1][mov.y1]='P'+dif;
        pos.fifty = 0;
    }
    else if(mov.type=='E'){ //en passant
        pos.flags[5]='-';
        pos.pos[mov.x][mov.y]='_';
        pos.pos[mov.x1][mov.y1]='P'+dif;
    ...
    else if(mov.type-'0'==0){ //rook moves
        pos.flags[5]='-';
        pos.flags[2+side*2]=' ';
        pos.pos[mov.x][mov.y]='_';
    ...
    else if(mov.type-'0'==7){ //other rook moves
        pos.flags[5]='-';
        pos.flags[1+side*2]=' ';
        pos.pos[mov.x][mov.y]='_';
        pos.pos[mov.x1][mov.y1]=='_' ? (pos.fifty++) : (pos.fifty=0);
...
std::pair<move1, int> move1::stupidMode(const board &pos, int depth) {
    if (depth == 0){
        ...
        if(moves.size()>6){
            std::sort(moves.begin(), moves.end(), [pos](move1 &a, move1 &b)-> bool{
                int sd = cof(pos.flags[0]);
                board t1(pos);
                playMove(t1, a);
                board t2(pos);
                playMove(t2, b);
                int e1 = stupidEval(t1);
                int e2 = stupidEval(t2);
                return sd*e1 > sd*e2;
            });
            moves.resize(6, moves[0]);
        }
        ...
        for(move1 m: moves){
            board tmp(pos);
            playMove(tmp,m);
            evals.push_back(stupidMode(tmp, depth-1).second);
        }
